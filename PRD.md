...
Status: V1 built and deployed 2026-08-05, live at t2s.joeleilat.com. Owner: Joel Eilat. Post-launch UX refinements (same day): API key collapses once saved, per-model approximate cost shown, player auto-scrolls into view, voice lists sorted alphabetically.
...
- **Providers/models/voices:** choose provider, then a model within it, then a voice. Models are labeled with capability hints (Hebrew support, speed/quality, preview status) and an approximate cost per 1,000 characters. Voices are listed alphabetically. ElevenLabs voices come from the user's own account.
- **Remembers selections:** the last-used provider, per-provider model and voice, and playback speed persist in localStorage and are restored on the next visit. Voice is remembered per provider (all of a provider's models share one voice set).
...
- **Keys:** entered per provider, stored only in browser localStorage, shown/hidden, saved explicitly; once saved the input collapses to a compact "Saved / Change" row. No cookies.
...
- No audio or user content persisted anywhere, and nothing stored server-side; only the API keys and UI preferences (provider/model/voice/speed) live in the browser's localStorage. Keys never leave the browser except in the request to the chosen provider.
